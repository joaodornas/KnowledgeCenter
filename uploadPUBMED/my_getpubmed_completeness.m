...
period_serial = first_day.serialDate:last_day.serialDate;

% total_days = length(period);

total_days_serial = length(period_serial);

% my_dates = zeros(total_days,3);

my_dates_serial = zeros(total_days_serial,3);

for iD=1:total_days_serial
    % for iD=1:total_days
    
    % this_day = period(iD);
    
    this_day_serial = period_serial(iD);
    
    % my_dates(iD,1) = this_day.Day;
    % my_dates(iD,2) = this_day.Month;
    % my_dates(iD,3) = this_day.Year;
    
    SerialDate = DateTime.initWithSerialDate(this_day_serial);
    
    my_dates(iD,1) = SerialDate.day;
    my_dates(iD,2) = SerialDate.month;
    my_dates(iD,3) = SerialDate.year;
    
end

% nDates = length(my_dates);

nDates = length(my_dates_serial);
